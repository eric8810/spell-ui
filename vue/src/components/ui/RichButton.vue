...
const buttonClass = computed(() =>
  cn(
    'inline-flex cursor-pointer items-center justify-center gap-2 whitespace-nowrap border border-zinc-950/35 bg-linear-to-b font-medium inset-shadow-2xs inset-shadow-white/25 shadow-md shadow-zinc-950/20 ring-0 transition-[filter] duration-200 hover:brightness-110 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring active:brightness-95 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 dark:border-0 dark:border-zinc-950/50 dark:bg-linear-to-t',
    colorMap[props.color],
    sizeMap[props.size],
    props.color === 'default' ? 'text-white dark:text-zinc-900' : 'text-white',
    props.class,
  ),
)

const contentClass = computed(() =>
  cn('relative inline-flex items-center gap-2', textShadowMap[props.color]),
)
</script>

<template>
  <Slot v-if="asChild" data-slot="rich-button" :class="buttonClass" v-bind="$attrs">
    <slot />
  </Slot>
  <button
    v-else
